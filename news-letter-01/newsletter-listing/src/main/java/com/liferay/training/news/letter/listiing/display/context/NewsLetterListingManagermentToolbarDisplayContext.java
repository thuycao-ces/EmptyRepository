        ...
        return new NavigationItemList() {
			{
				add(navigationItem -> {
					...
					navigationItem.setHref(getPortletURL(), "tabs1", "all");
					navigationItem.setLabel(LanguageUtil.get(_httpServletRequest, "all"));
				});

				add(navigationItem -> {
					navigationItem.setActive(Objects.equals(getTabs1(), "registration"));
					navigationItem.setHref(getPortletURL(), "tabs1", "registration");
					navigationItem.setLabel(LanguageUtil.get(_httpServletRequest, "registration"));
				});

				add(navigationItem -> {
					navigationItem.setActive(Objects.equals(getTabs1(), "login"));
					navigationItem.setHref(getPortletURL(), "tabs1", "login");
					navigationItem.setLabel(LanguageUtil.get(_httpServletRequest, "login"));
				});
			}
        };
    }
